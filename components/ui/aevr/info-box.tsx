...
const customInfoBoxVariants = cva(
  "relative flex grow flex-wrap items-start border transition-colors duration-200",
  {
    ...
    compoundVariants: [
      // Full color scheme variants
      {
        ...
        colorScheme: "full",
        className:
          "bg-yellow-50 dark:bg-yellow-950/10 border-yellow-200 dark:border-yellow-800/30 text-yellow-900 dark:text-yellow-100",
      },
      {
        type: "error",
        colorScheme: "full",
...
interface InfoBoxProps
  ...
  VariantProps<typeof customInfoBoxVariants> {
  loading?: boolean;
  icon?: ReactNode;
...
const CustomInfoBox: FC<InfoBoxProps> = ({
  loading,
  icon,
  title,
  description,
  ...
  return (
    <div
      className={`${cn(
        customInfoBoxVariants({ type, size, colorScheme }),
        className
      )} flex-col! p-3!`}
      {...props}
    >
      <div className="flex items-center gap-3">
        ...
        {title && <h3 className={`${titleVariants({ size })} dark:text-modernblack-05`}>{title}</h3>}
      </div>

      <div className="flex-1">
        {description && (
          <div className={`${descriptionVariants({ size })} dark:text-modernblack-05`}>
            {description}
          </div>
        )}
...
export { CustomInfoBox, customInfoBoxVariants };
